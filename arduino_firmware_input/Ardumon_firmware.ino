...
void setup() {
  ...
  ADC->ADC_CHER = PIN_MAP; // enable channels  (see page 1338 of datasheet) from 7 till 0 (pin A0-A7) 
...
void loop() {
  ...
  if (SerialUSB.available() > 0) // polls whether anything is ready on the read buffer - nothing happens until there's something there
  {
    ...
    if (incoming=='a'){
      digitalWrite(ledPin,!digitalRead(ledPin));   // Can be done faster toggling if PIND used
    }
